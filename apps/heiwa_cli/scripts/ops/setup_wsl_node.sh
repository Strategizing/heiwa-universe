...
ExecStart=$ROOT/apps/heiwa_cli/scripts/ops/worker_manager_daemon.sh
Restart=always
...
EOF"
    ...
    echo "⚠️  systemd not detected in WSL. Use './apps/heiwa_cli/scripts/ops/start_worker_stack.sh' manually."
fi

echo "✨ WSL Node Setup Complete."
echo "👉 Now run: cp .env.workstation .env.worker.local"
echo "👉 Then start: ./apps/heiwa_cli/scripts/ops/start_worker_stack.sh"
