...
def compile_targets() -> list[str]:
    errors: list[str] = []
    targets = [
        ROOT / "apps/swarm-hub/discord_portal.py",
        ROOT / "apps/swarm-hub/agents/messenger.py",
...
def check_env_hooks() -> list[str]:
    issues: list[str] = []
    config_path = ROOT / "apps/swarm-hub/config.py"
    if not config_path.exists():
        return ["missing file: apps/swarm-hub/config.py"]
    text = config_path.read_text(encoding="utf-8")
    for key in ("DISCORD_BOT_TOKEN", "DISCORD_GUILD_ID"):
        if key not in text:
            issues.append(f"apps/swarm-hub/config.py missing env reference: {key}")
    return issues
