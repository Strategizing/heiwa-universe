...
start_openclaw_gateway() {
    ...
    "$py_bin" "$ROOT/apps/heiwa-cli/scripts/ops/sync_openclaw_auth.py" || true
...
start_worker_manager() {
    ...
    if pgrep -f "apps/heiwa-cli/scripts/agents/worker_manager.py" >/dev/null 2>&1; then
        echo "[OK] worker_manager already running"
        return
    fi
    echo "[INFO] Starting worker_manager daemon wrapper ..."
    nohup "$ROOT/apps/heiwa-cli/scripts/ops/worker_manager_daemon.sh" >"$ROOT/runtime/logs/worker_manager.log" 2>&1 &
    sleep 1
    if pgrep -f "apps/heiwa-cli/scripts/agents/worker_manager.py" >/dev/null 2>&1; then
...
start_mini_thinker() {
    ...
    if pgrep -f "apps/worker-nodes/mini_thinker.py" >/dev/null 2>&1; then
    ...
    local py_bin
    py_bin=$(choose_python)
    nohup "$py_bin" "$ROOT/apps/worker-nodes/mini_thinker.py" >"$ROOT/runtime/logs/mini_thinker.log" 2>&1 &
    sleep 1
    if pgrep -f "apps/worker-nodes/mini_thinker.py" >/dev/null 2>&1; then
...
"$(choose_python)" "$ROOT/apps/heiwa-cli/scripts/ops/heiwa_360_check.py" || true
