...
def check_required_paths() -> list[str]:
    required = [
        ROOT / "apps/swarm-hub/main.py",
        ROOT / "apps/swarm-hub/config.py",
        ROOT / "apps/heiwa-cli/scripts/agents/sentinel.py",
...
def check_wrapper_flags() -> list[str]:
    issues: list[str] = []
    codex_wrapper = ROOT / "apps/heiwa-cli/scripts/agents/wrappers/codex_exec.sh"
    if codex_wrapper.exists():
        text = codex_wrapper.read_text(encoding="utf-8")
        if "--approval-mode" in text:
            issues.append("apps/heiwa-cli/scripts/agents/wrappers/codex_exec.sh uses deprecated --approval-mode flag")
    return issues
