...
class BaseAgent(ABC):
    ...
    async def connect(self, nats_url: str = None, max_retries: int = 10, retry_delay: int = 5):
        ...
        print(f"DEBUG: [{self.name}] Attempting to connect to NATS at {url}")
        
        for attempt in range(1, max_retries + 1):
            try:
                print(f"DEBUG: [{self.name}] Connection attempt {attempt}...")
                self.nc = await nats.connect(url, connect_timeout=10)
                print(f"DEBUG: [{self.name}] Successfully connected to NATS!")
