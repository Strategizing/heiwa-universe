...
def main():
    ...
    if not pages_domains:
        ...
        for d in pages_domains:
            host = d.get("host")
            # We assume project is "heiwa-clients" for this bootstrap logic
            # You could parse this dynamically if needed
            project_name = "heiwa-clients"
            if "docs" in d.get("purpose", "").lower():
                # Or a dedicated docs project if defined, but manifest currently points to "heiwa-clients" or "Cloudflare Pages docs project"
    ...
    print("   2. cd infrastructure/terraform/cloudflare")
