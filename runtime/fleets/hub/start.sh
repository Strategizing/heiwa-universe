...
run_tailscale_up() {
    if command -v timeout &>/dev/null; then
        timeout "$TS_TIMEOUT_SEC" tailscale --socket=/tmp/tailscaled.sock up --ephemeral --ephemeral --authkey="$AUTH_KEY" --hostname="$TS_HOSTNAME"
    elif command -v gtimeout &>/dev/null; then
        gtimeout "$TS_TIMEOUT_SEC" tailscale --socket=/tmp/tailscaled.sock up --ephemeral --ephemeral --authkey="$AUTH_KEY" --hostname="$TS_HOSTNAME"
    else
        tailscale --socket=/tmp/tailscaled.sock up --ephemeral --ephemeral --authkey="$AUTH_KEY" --hostname="$TS_HOSTNAME"
    fi
}
